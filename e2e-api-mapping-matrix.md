...
| `TestModeratePage_AddSpeakerFromAttendeesCard` in [moderate_test.go](/home/patric/Projects/conference-tool/e2e/moderate_test.go#L156) | Moderator searches the attendee roster inline and adds a speaker from the moderation screen | `/committee/[slug]/meeting/[meetingId]/moderate` | `ModerationService.GetModerationView`, `AttendeeService.ListAttendees`, `SpeakerService.ListSpeakers`, `SpeakerService.AddSpeaker`, `GET /api/realtime/meetings/{meetingId}/events` | `TestAttendeeService_ListAttendees`, `TestSpeakerService_AddSpeaker_ChairpersonAddsMeetingAttendee`, `TestMeetingEvents_SpeakersUpdated_RefetchesModerationAndLive` | The moderation screen now owns its candidate-search workflow using a typed attendee roster instead of server-rendered fragments | `mapped` |
| `TestModeratePage_QuickControls` in [moderate_test.go](/home/patric/Projects/conference-tool/e2e/moderate_test.go#L119) | Moderator starts the next speaker and ends the current speech from the moderation screen | `/committee/[slug]/meeting/[meetingId]/moderate` | `ModerationService.GetModerationView`, `SpeakerService.ListSpeakers`, `SpeakerService.SetSpeakerSpeaking`, `SpeakerService.SetSpeakerDone`, `GET /api/realtime/meetings/{meetingId}/events` | `TestSpeakerService_SetSpeakerSpeaking_ThenDone`, `TestMeetingEvents_SpeakersUpdated_RefetchesModerationAndLive` | The SPA moderation screen now owns the queue controls directly instead of relying on SSR-only manage fragments | `mapped` |
| `TestAttendeeLive_SelfAddButtons` in [attendee_speakers_test.go](/home/patric/Projects/conference-tool/e2e/attendee_speakers_test.go#L138) | Attendee adds themselves to the speakers queue from the live meeting page | `/committee/[slug]/meeting/[meetingId]` | `MeetingService.GetLiveMeeting`, `SpeakerService.ListSpeakers`, `SpeakerService.AddSpeaker`, `GET /api/realtime/meetings/{meetingId}/events` | `TestSpeakerService_AddSpeaker_SelfForAttendeeSession`, `TestMeetingEvents_SpeakersUpdated_RefetchesLive` | This is the first attendee-side speaker command now wired through the SPA | `mapped` |
| `TestAgendaPoint_CreateAndShow` in [agenda_speakers_test.go](/home/patric/Projects/conference-tool/e2e/agenda_speakers_test.go#L128) | Moderator creates a top-level agenda point and sees it appear inline | `/committee/[slug]/meeting/[meetingId]/moderate` | `ModerationService.GetModerationView`, `AgendaService.ListAgendaPoints`, `AgendaService.CreateAgendaPoint`, `GET /api/realtime/meetings/{meetingId}/events` | `TestAgendaService_CreateAndDelete`, `TestMeetingEvents_AgendaUpdated_RefetchesModerationAndLive` | The SPA moderation screen now includes the first typed agenda-management form for top-level agenda items | `mapped` |
| `TestAgendaPoint_Activate` in [agenda_speakers_test.go](/home/patric/Projects/conference-tool/e2e/agenda_speakers_test.go#L192) | Moderator activates or clears the active agenda point from the agenda list | `/committee/[slug]/meeting/[meetingId]/moderate` | `ModerationService.GetModerationView`, `AgendaService.ListAgendaPoints`, `AgendaService.ActivateAgendaPoint`, `GET /api/realtime/meetings/{meetingId}/events` | `TestAgendaService_ActivateAgendaPoint`, `TestAgendaService_ActivateAgendaPoint_MemberForbidden`, `TestMeetingEvents_AgendaUpdated_RefetchesModerationAndLive` | Activation now uses the typed agenda command and feeds back into both the moderation summary and live meeting read model | `mapped` |
| `TestAgendaPoint_ReorderMoveUp` in [agenda_speakers_test.go](/home/patric/Projects/conference-tool/e2e/agenda_speakers_test.go#L269) | Moderator reorders top-level agenda points from the agenda list | `/committee/[slug]/meeting/[meetingId]/moderate` | `AgendaService.ListAgendaPoints`, `AgendaService.MoveAgendaPoint`, `GET /api/realtime/meetings/{meetingId}/events` | `TestAgendaService_MoveAgendaPoint_ReordersTopLevelAgenda`, `TestMeetingEvents_AgendaUpdated_RefetchesModerationAndLive` | This first slice ports top-level reordering only; sub-point editing and import remain later agenda work | `mapped` |
